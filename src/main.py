...
def calculate_investment():
    ...
    for investment_year in range(1, input["years_to_invest"]["years"] + 1):
        ...
        for currency, yearly_interest_rate in input["yearly_interest_rate"].items():
            ...
            prt(f"    Gained as interest: {fmt_float(gained_as_interest)} {currency}")

            total_accumulation[currency] += gained_as_interest
            prt(f"    Total: {fmt_float(total_accumulation[currency])} {currency}")

            if input["yearly_investment"].get(currency):
                prt(f"    Yearly investment: {input["yearly_investment"][currency]} {currency}")
        ...
        prt(f"    Total: {fmt_float(total_in_output_currency)} {input["output_currency"]}")
